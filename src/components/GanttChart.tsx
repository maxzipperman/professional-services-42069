...
export default function GanttChart({ tasks }: GanttChartProps) {
  const { parsed, invalid } = useMemo(() => {
    const invalid: GanttTask[] = [];
    const parsed = tasks.reduce<(GanttTask & { s: Date; e: Date; })[]>((acc, t) => {
      const s = new Date(t.start);
      const e = new Date(t.end);
      if (e < s) {
        console.warn(`GanttChart: task "${t.label}" has end date before start date.`);
        invalid.push(t);
        return acc;
      }
      acc.push({ ...t, s, e });
  ...
  const minDate = useMemo(() => (
    parsed.length ? new Date(Math.min(...parsed.map(t => t.s.getTime()))) : new Date()
  ), [parsed]);
  const maxDate = useMemo(() => (
    parsed.length ? new Date(Math.max(...parsed.map(t => t.e.getTime()))) : new Date()
  ), [parsed]);
  ...
  if (parsed.length === 0) {
    return invalid.length > 0 ? (
      <Alert variant="destructive">
        <AlertTitle>Invalid tasks</AlertTitle>
        <AlertDescription>
          Some tasks have end dates before their start dates and were skipped.
        </AlertDescription>
      </Alert>
    ) : null;
  }
